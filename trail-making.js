...
// Función para obtener la dirección de correo
async function getEmailAddress() {
  try {
    const response = await fetch(LAMBDA_API_URL, {
      ...
      body: JSON.stringify({}) // Enviar una solicitud vacía para obtener la dirección de correo
    });
    if (!response.ok) {
      throw new Error('Error al obtener la dirección de correo');
    }
    const data = await response.json();
    return data.email;
  } catch (error) {
    console.error('Error al obtener la dirección de correo:', error);
    return null;
  }
}

// Función para enviar los resultados del experimento
async function sendExperimentResults() {
  ...
  if (!emailAddress) {
    alert('Hubo un error al obtener la dirección de correo. Por favor, inténtalo de nuevo.');
    return;
  }

  const emailData = {
    from_name: 'Tu Nombre', // Reemplaza con tu nombre o identificador
    to_name: emailAddress, // Usar la dirección de correo obtenida
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Error al comunicarse con la API Gateway');
    }
