...
function validatePhoneNumber(phoneNumber) {
  // Verificar que no esté vacío y contenga solo dígitos
  const phoneRegex = /^[0-9]{8,12}$/; // Asegurarse de que tenga entre 8 y 12 dígitos
...
async function showDialogAndValidate() {
  let result;
  do {
    result = await psychoJS.gui.DlgFromDict({
      dictionary: expInfo,
      title: expName,
    });

    if (psychoJS.gui.dialogComponent.button === 'OK') {
      if (!validatePhoneNumber(expInfo['Escribe tu teléfono, por favor'])) {
        alert('Por favor, ingresa un número de teléfono válido.');
      } else {
        return true;  // Número de teléfono válido, salir del bucle
      }
    } else {
...
showDialogAndValidate().then((continueExperiment) => {
