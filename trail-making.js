...
// Validar el correo electrónico después del cuadro de diálogo
psychoJS.schedule(() => {
  if (!validarCorreo()) {
    return Scheduler.Event.REPEAT; // Si el correo está vacío, repetir el cuadro
  }
...
flowScheduler.add(updateInfo); // Añadir timestamp y datos iniciales
...
async function updateInfo() {
  ...
  // Obtener la tasa de fotogramas
  expInfo['frameRate'] = psychoJS.window.getActualFrameRate();
  if (typeof expInfo['frameRate'] !== 'undefined') {
    frameDur = 1.0 / Math.round(expInfo['frameRate']);
  } else {
    frameDur = 1.0 / 60.0; // Valor predeterminado si no se mide la tasa
  }
